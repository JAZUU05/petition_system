...
Public Class edit_form
    ...
    Private Sub Edit_form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim img As Byte()
        sql = "SELECT petition_id, petition_num, year, ra, doc_owner, gender, name_of_petitioner, type_doc, 
                    municipality, province, certificate, certificate_2, certificate_3, year_final FROM petition_record WHERE  petition_id = '" & edit_id & "'"
    ...
    Sub edit()
        con.Open()
        sql = "UPDATE petition_record SET petition_num = @pet_num,year = @year, ra = @ra ,doc_owner = @doc_owner ,gender = @gender ,name_of_petitioner = @name_of_pet ,type_doc = @type_doc,
            municipality = @mun ,province = @prov ,certificate = @certificate, certificate_2 = @certificate_2, certificate_3 = @certificate_3, year_final = @year_final WHERE petition_id = '" & edit_id & "'"
        ...
        With cmd
            ...
            If res > 0 Then
                MsgBox("Updated Record", MsgBoxStyle.Information, "Record Updated")
